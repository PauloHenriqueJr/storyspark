...
// Import configs modulares
import { socialAndMarketingConfigs } from './configs/socialAndMarketingConfigs';
import { personasAndVoicesConfigs } from './configs/personasAndVoicesConfigs';
import { communicationConfigs } from './configs/communicationConfigs';
import { calendarAndAnalyticsConfigs } from './configs/calendarAndAnalyticsConfigs';
import { otherConfigs } from './configs/otherConfigs';
...
const FloatingCopyButton: React.FC<FloatingCopyButtonProps> = ({ toastNotifications, systemToastNotifications, onOpenScheduleModal }) => {
  ...
  // Excluir admin, auth, settings, team, integrations, billing, blog, not-found, test pages
  const excludedPaths = [
    '/admin',
    '/auth',
    '/auth-callback',
  ...
  const isExcluded = excludedPaths.some(path => location.pathname.startsWith(path)) || isAdminPath;
