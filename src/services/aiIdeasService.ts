...
type GeneratedCopyInsert = Database['public']['Tables']['generated_copies']['Insert'];
...
export const aiIdeasService = {
  ...
  async saveIdeas(
    ...
    if (!workspaceId || !userId) {
      throw new Error('WorkspaceId e userId são obrigatórios para salvar ideias.');
    }
    ...
    const rows: GeneratedCopyInsert[] = ideas.flatMap((idea) => {
      ...
      const channel = (platform || category || 'ideas').toString().toLowerCase();
      ...
      return (content || []).map((text) => {
        ...
        const row: GeneratedCopyInsert = {
          ai_model: 'ideas-v1',
          ai_provider: 'local',
          brief: brief as unknown as Database['public']['Tables']['generated_copies']['Insert']['brief'],
          channel,
          content: text,
          credits_used: 1,
          is_favorite: false,
  ...
  async fetchIdeas(workspaceId: string, limit = 50) {
    ...
    return (data || []).map((row: any) => {
      const brief = (row.brief || {}) as any;
      const text: string = row.content || '';
      const titleFromText = text ? text.slice(0, 60) + (text.length > 60 ? '...' : '') : 'Ideia';
      return {
        id: row.id,
