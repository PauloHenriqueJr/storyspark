...
class EmailQueueService {
  ...
  public async processQueue(): Promise<void> {
    ...
    try {
      ...
      const validEmails = pendingEmails.filter(email => email.attempts < email.max_attempts);
  ...
  private async processEmailItem(emailItem: EmailQueueItem): Promise<void> {
    try {
      ...
      await this.updateEmailStatus(emailItem.id, 'processing', emailItem.attempts + 1);
      ...
      switch (emailItem.email_type) {
        ...
        case 'waitlist_invite':
          ...
          result = { success: false, error: 'Tipo de email não implementado ainda' };
          break;
        ...
        default:
          result = { success: false, error: `Tipo de email desconhecido: ${emailItem.email_type}` };
      }
      ...
      if (result.success) {
        ...
        console.log(`Email ${emailItem.email_type} enviado com sucesso para ${emailItem.email}`);
      } else {
        const newAttempts = emailItem.attempts + 1;
        const status = newAttempts >= emailItem.max_attempts ? 'failed' : 'pending';
        ...
        if (status === 'failed') {
          console.error(`Email ${emailItem.email_type} falhou definitivamente para ${emailItem.email}: ${result.error}`);
        } else {
          console.warn(`Email ${emailItem.email_type} falhou (tentativa ${newAttempts}/${emailItem.max_attempts}) para ${emailItem.email}: ${result.error}`);
        }
      }
    } catch (error: any) {
  ...
  private async sendWaitlistConfirmation(emailItem: EmailQueueItem): Promise<{ success: boolean; messageId?: string; error?: string }> {
